...
export const TriangleArrowDownIcon: React.FC<TriangleArrowDownIconProps> = ({
  className,
  ...props
}) => {
  const { width, height, viewBox, scaleFactor, iconStyles } = useIcon(props);

  const triangleArrowDownIconStyles = css({
    '& circle': {
  ...
  return (
    <svg
      ...
      viewBox={viewBox}
      fill="none"
      className={`${iconStyles} ${triangleArrowDownIconStyles} ${className || ''}`}
    >
